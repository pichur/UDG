...
from discrete_disk_binary import create_area_by_join, DiscreteDisk, Coordinate, DISK_NONE
...
class Graph:
    ...
    def __init__(self, n_or_g):
        ...
        self.coordinates = [
            Coordinate(np.int64(0), np.int64(0), False)
    ...
    def set_coordinate(self, v: int, x: int, y: int, value: bool = False):
        """Set coordinates for vertex ``v``."""
        v = self.coordinates[v]
        v.x, v.y, v.value = x, y, value
        return self
        ...
        for i in range(self.n):
            for j in range(i + 1, self.n):
                for d in range(discrete_disk.RO[self.unit] * self.n):
                    if self.vertex_distances[i][j][d]:
    ...
    def has_discrete_realization(self):
        ...
        for only_I in range_modes:
            ...
            count_I: int = 0
            result = self.place_next_vertex(0, False, only_I, count_I)
            if result == YES:
                return YES
        return result
    ...
    def calculate_vertex_distances(self):
        self.result_I = 0
        self.result_B = 0
        self.vertex_distances = [[[False for _ in range(discrete_disk.RO[self.unit] * self.n)] for _ in range(self.n)] for _ in range(self.n)]
    ...
    def calculate_node_distances(self, nodes: tuple[int, int], order: str = "DD") -> list[int]:
        ...
        # +1 for 0 +1 for max
        self.node_distances = [False] * (maximum_vertex_edge_distance * self.unit + 2)
        ...
        for only_I in [True]:
            ...
            while self.check_distance:
                ...
                count_I: int = 0
                r_text = self.place_next_vertex(0, False, only_I, count_I)
                r_mode = False
                if r_text == YES:
                    r_mode = True
                else:
                ...
                if dx >= len(self.node_distances):
                    self.node_distances.extend([False] * (dx - len(self.node_distances) + 1))

                if self.node_distances[dx] != True:
    ...
    def store_vertex_distances(self, value: bool = False):
        ...
        for i in range(self.n):
            for j in range(i + 1, self.n):
                ...
                if self.vertex_distances[i][j][distanceF] != True:
                    self.vertex_distances[i][j][distanceF] = value
                if self.vertex_distances[i][j][distanceC] != True:
                    self.vertex_distances[i][j][distanceC] = value
    ...
    def place_next_vertex(self, j: int, calc_D: bool, only_I: bool, count_I: int):
        if self.order[j] == -1:
            ...
            for v in range(self.n):
                ...
                P = self.candidate_points(j, only_I, count_I)
        ...
        for p in P:
            ...
            incr_I = 1 if p.value else 0

            self.set_coordinate(vertex, p.x, p.y, p.value)
            ...
            if j < self.n - 1:
                result = self.place_next_vertex(j + 1, calc_D, only_I, count_I + incr_I)
                if result == YES:
                    return YES
            else:
                ...
                if count_I + incr_I == self.n:
                    if calc_D:
                        self.store_vertex_distances(True)
                        self.result_I += 1
                    else:
                        return YES
    ...
    def state_info(self, only_I: bool, j:int) -> str:
        ...
        for i in range(j+1):
            ...
            x = c.x
            y = c.y
            info += f"  [{i:4d}/{s:4d}] {k} {'+' if c.value else '-'} ({x: =3d}:{y: =3d})"
        return info
    
    def is_udg_realization(self) -> bool:
        return all(coord.value for coord in self.coordinates)
    
    def candidate_points(self, j: int, only_I: bool, count_I: int) -> list[Coordinate]:
        P = self.candidate_points_without_order(j, only_I, count_I)
    ...
    def candidate_points_without_order(self, j: int, only_I: bool, count_I: int) -> list[Coordinate]:
        if j == 0:
            P = []
            P.append(Coordinate(x = 0, y = 0, value = True))
            return P
        if j == 1:
            ...
            if self.limit_points:
                P = [p for p in area.points_iter() if p.y == 0 and p.x >= 0]
            else:
                P = [p for p in area.points_iter() if p.y >= 0 and p.x >= 0]
        ...
        if j == 2:
            if self.limit_points:
                P = [p for p in area.points_iter() if p.y >= 0]
                return P
            else:
                return area.points_list()
        else: 
            return area.points_list()
